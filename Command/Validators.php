...
class Validators
{
    ...
    public static function validateFormat($format)
    {
     ...
     public static function validateYesNo($decision)
    {

        if (!in_array($decision, array('yes','y',true, 'no','n',false))) {
            
            if(in_array($decision,['no','n',false]))
            {
                $decision=false;
            }
            else
            ...
            throw new \RuntimeException(sprintf('Decizion must be yes or no', $decision));
        }
